...
for i = 1:400
    ...
    Re = (G*Dh)/mu_l;
    Phi = 1.826*(P/D) - 1.0430;
    Pr_l = interp1(Tsat,Pr_f,T_m(i),'linear','extrap');
    Nu = Phi*(0.023*(Re^0.8)*(Pr_l^0.333));
    ...
    elseif T_m(i) >= T_sat
         ...
         elseif sub == 1 
            ...
            if h_feb < h_fsat
                x_e(i) = (h_feb-h_fsat)/(h_gsat-h_fsat);
    ...
    %Bowring
    if boiling == 1
        pr = 0.145*Pnom; %dont have to worry about pr < 1
        n = 0.5*pr;
    ...
    elseif boiling == 1
        ...
        dP(i) = ((f_lo*(1/Dh)*((G^2)/(2*rho_l))+dPa+dPg))*del_z;
        table.dP(i) = dP(i);
    end
    %T_ci
    R_ci = D_ci/2;
